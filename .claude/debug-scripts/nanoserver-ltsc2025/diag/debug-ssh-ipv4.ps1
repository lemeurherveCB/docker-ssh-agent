$port = (docker port pester-jenkins-ssh-agent-nanoserver-ltsc2025-jdk21 22 2>$null) -replace '.*:',''
if (-not $port) { Write-Host "ERROR: no container port"; exit 1 }
Write-Host "Port: $port"

$key = @"
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
"@

$keyFile = [System.IO.Path]::GetTempFileName()
[System.IO.File]::WriteAllText($keyFile, $key)
icacls $keyFile /inheritance:r /grant:r "${env:USERNAME}:(R)" | Out-Null

Write-Host "=== SSH via 127.0.0.1 (IPv4) ==="
$proc = Start-Process -FilePath "C:\Windows\System32\OpenSSH\ssh.exe" `
  -ArgumentList "-v -4 -i `"$keyFile`" -o UserKnownHostsFile=NUL -o StrictHostKeyChecking=no -o ConnectTimeout=15 -o BatchMode=yes -l jenkins 127.0.0.1 -p $port pwsh.exe -NoLogo -C `"Write-Host f00`"" `
  -NoNewWindow -PassThru -RedirectStandardError "C:\ssh-ipv4-stderr.txt" -RedirectStandardOutput "C:\ssh-ipv4-stdout.txt"
$done = $proc.WaitForExit(20000)
if (-not $done) { Write-Host "HUNG"; $proc.Kill() } else { Write-Host "Exit: $($proc.ExitCode)" }
Write-Host "=== stdout ==="
Get-Content "C:\ssh-ipv4-stdout.txt" -ErrorAction SilentlyContinue
Write-Host "=== stderr ==="
Get-Content "C:\ssh-ipv4-stderr.txt" -ErrorAction SilentlyContinue

Write-Host "=== sshd.log last 15 lines ==="
docker exec pester-jenkins-ssh-agent-nanoserver-ltsc2025-jdk21 pwsh.exe -NonInteractive -C "Get-Content C:\\ProgramData\\ssh\\logs\\sshd.log -ErrorAction SilentlyContinue | Select-Object -Last 15"

Remove-Item $keyFile -Force
